...
//		List<AttchVO> attachList = new ArrayList<>();
	...
	private void uploadHwkAttatch(HwkVO hwk) {
		...
		try {			
			attach.saveTo(saveHwFolder);
			log.info("saveHwFolder..................{}",saveHwFolder);
		}catch(IOException e) {
			throw new RuntimeException(e);
		}
	...
	public ServiceResult createGradingHwk(HwkVO hwk) {
		...
		int rowcnt = dao.updateGradingHwk(hwk);
	...
	public LecHomeworkVO retrieveMainHW(String lecId) {
		int lechwkNo = dao.selectMaxLechhwkNo(lecId);
		LecHomeworkVO homework = null;
		if(lechwkNo != 0) {
				homework = dao.homework(lechwkNo);			
		}
