		  ...
		  <div class="container-fluid">
		    <div class="collapse navbar-collapse" id="navbarText">  
		      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
		        ...
		        <li class="nav-item">
		          <a class="nav-link active" href="${cPath }/lecBoard/qna">질문과 답변</a>
		        </li>
		        <li class="nav-item">
...
<br>
<h3 class="h3-title">질문답변</h3><hr class="hr-title">

<button type="button" class="btn btn-primary btn-sm" onclick="location.href='${cPath}/lecBoard/qna/new'">글쓰기</button>
...
<!-- 				<h4>검색용 UI</h4> -->
				...
				<div id="searchDIV">
					<select name="searchType"  class="form-select" id="inlineFormSelectPref">
						<option>전체</option>
						<option value="TITLE">제목</option>
						<option value="CONTENT">내용</option>
					</select> 
					<input type="text" id="searchWord" name="searchWord" class="form-control mr-2" />
...
<script type="text/javascript">
	const VIEWURLPTRN = CONTEXTPATH+"/lecBoard/qna/%V";
		...
		, success:function(paging){
			...
			if(boardList && boardList.length > 0){
				$.each(boardList, function(idx, board){
					...
					if(board.proName == null){
						userName = board.stuName
					}else{
						userName = board.proName
					}
